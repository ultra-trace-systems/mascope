...
def fit_mass_accuracy(
    ...
    :return: The offset, and the width - or ``None`` for a width where fewer
        than :data:`MASS_ACCURACY_MIN_ANCHORS` anchors were given, which is not
        a small measurement of a width but no measurement of one.
    """
...
def fit_sample_mass_accuracy(
    ...
    :param match_isotope_df: A match frame, as ``compute_match_isotopes``
        produces it.
    :return: The offset, and the width or ``None`` - including for a frame that
        carries neither column, which has not measured a mass error, and that
        is the same answer as a frame carrying too few.
    """
