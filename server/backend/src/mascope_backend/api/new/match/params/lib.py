...
def _instrument_types_in(df: pd.DataFrame) -> dict[str, str]:
    ...
    if "instrument_type" in df.columns:
        # Sorted before the drop so a name that carries two classes - refused
        # at upload, but possible in older data - settles the same way on
        # every call instead of following the frame's row order.
        pairs = (
            df[["instrument", "instrument_type"]]
            .dropna()
            .sort_values(["instrument", "instrument_type"])
