...
async def get_instruments() -> dict:
    ...
    instrument can be called anything. One instrument is one class: an upload
    whose data file contradicts the name's own class is refused, so any of a
    name's files answers for all of them. The lowest is taken so that a name
    which carried both before that was refused still lists once, and always
    as the same one.
