...
def _reject_oversized_upload(metadata: dict, upload_info: dict) -> None:
    """Refuse to create a tus upload whose declared length exceeds the cap.

    Fails the creation request fast with 413 so well-behaved clients report
    a clear error. Uploads that defer their length (or lie about it) are
    bounded by tuspyserver's streaming max_size enforcement instead, which
    stops writing at the cap.
    """
    size = upload_info.get("size")
    if size is not None and size > _tus_max_upload_bytes:
        raise HTTPException(
            status_code=413,
            detail=(
                f"Upload of {size} bytes exceeds the per-upload limit of "
                f"{runtime.config.tus_max_upload_gb} GB"
            ),
        )
...
sample_files_upload_router = create_tus_router(
    ...
    # POST and PATCH authenticate through get_upload_handler; auth= covers
    # the remaining generated routes (HEAD, OPTIONS, DELETE), which
    # tuspyserver otherwise serves without any authentication.
