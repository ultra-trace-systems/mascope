...
class _FlakyH5Processor(H5Processor):
    """An h5 processor whose reader answers once and then stops."""
    ...
    def timestamp(self) -> str:
        ...
        if type(self).reads > 1:
            raise OSError("the file could not be opened again")
        return "2026-09-05T10:12:01"


def test_the_name_is_settled_once_and_does_not_change_under_the_filestore():
    # `filename` is read again after the filestore directory has been created
    # under the first answer. Re-deriving it would re-open the file, and a
    # transient failure there would hand back the unstamped stem - a different
    # name, an orphaned directory, and a retry that collides with it.
    processor = _FlakyH5Processor(
        socket_client=None, file_queue=Queue(), shutdown_event=Event()
    )
    processor.file_to_process = "/data/Test_ambient.h5"

    first = processor.filename
    assert first == "Test_2026.09.05-10h12m01s_ambient"
    assert processor.filename == first
    assert _FlakyH5Processor.reads == 1
