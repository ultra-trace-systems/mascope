...
then "tof" or "api". A name the rule cannot read is refused before the NOT
NULL, with the name in the message - the last test here - since a row that
predates the name validation, or that an update wrote directly, need not
pass the rule.
...
def test_a_name_the_rule_cannot_read_is_refused_by_name(
    seeded_alembic_config: Config, seeded_engine: Engine
):
    # Runs last: it takes the database back to PRIOR_REVISION for itself.
    # An unreadable name must stop the upgrade with something to act on, not
    # with a NOT NULL violation that names no row.
    downgrade(seeded_alembic_config, PRIOR_REVISION)
    with seeded_engine.begin() as conn:
        conn.execute(
            ...
            {
                "id": "sf-type-none",
                "filename": "Lab3_2026.09.01-12h00m00s_sf-type-none",
                "instrument": "Lab3",
                "local": datetime(2026, 9, 1, 12, 0),
                "utc": _BASE_TIME,
            },
    ...
    with pytest.raises(RuntimeError, match="Lab3"):
        upgrade(seeded_alembic_config, REVISION)

    # The failed upgrade rolled back, so the column is not there to fix; the
    # operator's route is to correct the name, which the migration then reads.
    with seeded_engine.begin() as conn:
        conn.execute(
            text(
                "UPDATE sample_file SET instrument = 'Orbi-Lab3' WHERE sample_file_id = :id"
            ),
            {"id": "sf-type-none"},
        )
    upgrade(seeded_alembic_config, REVISION)
    assert _types(seeded_engine)["sf-type-none"] == "orbi"
