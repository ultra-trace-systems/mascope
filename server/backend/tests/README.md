...
These tests run Alembic in-process, in the same session as the unit and system tests after them, so `env.py` leaves the session's logging alone and applies `alembic.ini`'s logging setup only when Alembic runs from its own command line. Its module docstring explains why; `test_env_logging.py` pins both cases.
