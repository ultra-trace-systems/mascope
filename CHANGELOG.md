...
- **The API's OpenAPI document is published with the user docs.** Every
  ...
  is rendered from the backend's routes when the frontend image is built, and
  names the version that image is built for as its `info.version` - in a
  throwaway prod runtime with placeholder secrets, so it is the same for every
  deployment of a release and carries nothing of the machine that built it -
  while the backend itself still serves its schema, `/docs` and `/redoc` in
...
- **Copy and paste work on a deployment served over plain HTTP.** A page
  ...
  command, as the About tab already did. A copy button that cannot copy either
  way says so, and a one-time password is still discarded only once it was
  really copied; a share link that cannot be copied either way is shown in the
  notification for copying by hand; and a dataset, batch or sample copied or
  cut in a tab can be pasted in that tab even where the clipboard cannot be
  read back, while a cut stops being offered in the other tabs once it is
  pasted.
