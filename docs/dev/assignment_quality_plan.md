...
| 3.1c - the stronger partner decides | #2205 | built: the partner gate keeps each neutral's partners with how strongly each commits it (tier, then peak height), current through every swap, and a reading through a gated channel that stands on a partner is weighed against every other reading of its ion through a gated channel that has one: the stronger partner takes the ion (`took_from`), a tie leaves the row its reading, and the row records what it weighed (`partner_gate.contest`: the partners' tiers and peak ratio). Where the tiers differ or the stronger peak is at least `PARTNER_MARGIN` (10) times the other's, the weaker reading is marked `outweighed` and the cross-channel pass reads the ion as settled by the partner (`same_ion_settled` by `partner`); within the margin it stays a rival, and the row is held at candidate. A reading through a mode channel is not contested; a set-aside reading the ledger later bears out is weighed rather than swapped back to; where a reading has no partner and the family's mode reading is no molecule, the gate takes the opportunistic reading with the strongest partner rather than the first. The cross-channel pass no longer lets a second channel settle a rival whose molecule the sample commits through a mode channel at candidate or better (`shown`, counted as `shown_rival`), on every row. Tiering rule set 7. Not yet measured on the testbed; the need measured on set K: the benzyl cation as protonated C7H6 at assigned in 6 of 6 files with toluene less a hydride on the row |
