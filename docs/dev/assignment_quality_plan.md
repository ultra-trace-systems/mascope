...
| 3.1 - a profile for the charge-transfer source | PR pending | built: `EASYIC_POS` and `EASYIC_NEG` resolve from the bare sign after every reagent (a reagent mode that also declares electron transfer keeps its reagent; a mode with only protonation or deprotonation stays ESI), both under the ambient context; the fluoranthene beam is the reagent ladder (the air-plasma cations wait for 3.3); hydride abstraction `-H-` and proton transfer (positive) and deprotonation (negative) are secondary channels switched on by the beam, protonated fluoranthene or hydronium, and the source's own deprotonated acids, and left on where a narrow window cannot show them; `parse_ionization` now reads `-H-` as the grammar and the validator do (a hydride removed, a cation) instead of as deprotonation. Sets I+ and I- to be measured on the testbed once the bulk run over the cloned dataset finishes |
