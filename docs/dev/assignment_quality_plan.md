...
- **As built (#2205, after its review).** Three departures from the text
  ...
  - *The shown-rival rule weighs as the contest does.* A rival the sample
    shows is settled where the row's own molecule, the row itself left out,
    is shown on a peak `PARTNER_MARGIN` times as bright or more, and is a
    doubt otherwise. Set L's rows, where toluene is 27 to 31 times C7H6, still
    fall to candidate.
