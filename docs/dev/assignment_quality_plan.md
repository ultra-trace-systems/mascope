...
hand-assigned row to repair. Still open from the same defect: the isotopologue
tables label a row from its formula's brackets (`formatIsotopeFormula`), so on
the 15N sets a labelled ion's M0 reads "[15N]" and its remainder "M0". Fixing
that needs the ion formula at each of the five call sites and is a separate
change.
