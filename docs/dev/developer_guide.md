...
Mascope docker images are built by a release CI/CD pipeline (found in `.github/workflows/release.yaml`). This pipeline triggers whenever we merge to master; you can follow [release workflow runs](https://github.com/ultra-trace-systems/mascope/actions/workflows/release.yaml). The pipeline builds and tags production images and pushes them to the GitHub Container Registry (ghcr.io). You can see these images in [Karsa's GitHub packages page](https://github.com/orgs/ultra-trace-systems/packages).
