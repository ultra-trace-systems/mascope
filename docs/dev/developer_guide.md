...
(`_docs_kwargs` in `app/fast.py`, checked by the pen-test suite's RECON-01/02).
A production deployment publishes a static copy of the document instead, next
to the user docs at `/docs/openapi.json`, and the SDK docs page links to it. It
is rendered when the frontend image is built (the `openapi-build` stage of
`server/frontend/Dockerfile`), and by the "Docs strict build" CI job, with:
...
the one that rendered it. Its `info.version` stays a placeholder, because the
running version is an admin-only read (`GET /api/version`).
