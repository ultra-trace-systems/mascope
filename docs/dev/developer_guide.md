...
Numbers in a response must be finite: JSON has no NaN or Infinity, so return
`None` for a missing value. `@api_route` renders with `NanSafeJSONResponse`
(`api/lib/api_features.py`): a non-finite float that slips through is sent as
`null`, and a WARNING naming the route is logged, which reaches error
monitoring so that the route gets fixed. Only floats are mapped, so text such
as a formula `"NaN"` is sent as it is.
...
```json
{
  ...
  "detail": {
    "error_id": "9f3c2a7e5b1d4c8e8a6f0b2d4e6a8c0f" // Matches the server log entry
  }
}
```

- `error`: Designed for client-side notifications. Display meaningful error messages directly to users.
- `detail`: Identifies the error: an `error_id` to quote when reporting it, which matches the server's log entry for it (with the traceback, for a server fault), and a stable `code` where a client is expected to react to the condition. A warning (status 200 or 207) carries its own data here. Tracebacks and internal messages are never sent to the client.

A route whose result cannot be rendered as JSON has failed, not the request:
`@api_route` answers it 500 and logs it at ERROR, whatever exception the
encoder raised (`ResponseRenderError` in `api/lib/exceptions/api_exceptions.py`).
