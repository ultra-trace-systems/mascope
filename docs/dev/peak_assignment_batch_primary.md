...
> | R | The sample browser's status badge is a column of its own after the sample name, and column reordering works (a key per column, the fixed-column offset, the table's private order cleared); a ledger-served peak's isotopologue abundances are fractions of the M0 | [#2065](https://github.com/ultra-trace-systems/mascope/pull/2065) |
> | S | The on-demand measurement reads like an isotope table: labels from the monoisotopic peak, abundances as fractions of the most abundant isotopologue, and a *main peak* note (naming the nearest prediction) instead of a false "not measured" when nothing paired with the family's main peak | [#2067](https://github.com/ultra-trace-systems/mascope/pull/2067) |
> | T | One vocabulary: the engine's M0 - the family head that carries the assignment, the reference the labels count from, the ledger owner - is the monoisotopic isotopologue rather than the most abundant one, and the inspector reads a run's abundances as fractions of the family's tallest member | [#2069](https://github.com/ultra-trace-systems/mascope/pull/2069) |
>
